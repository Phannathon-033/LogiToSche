...
Extracted from slm_app.py without PyTorch/CUDA dependencies so it can run alongside PaddleOCR.
"""
import re
...
def is_grounded_in_ocr(val: Any, ocr_text: str) -> bool:
    ...
    # Multi-word substring overlap check
...
def evaluate_11_fields(ocr_text: str) -> dict[str, Any]:
    """Evaluate 11 core logistics fields strictly grounded from OCR text."""
    ...
    unit_price, _ = parse_grounded_unit_price(ocr_text)
    ...
    v_curr = curr_code if (curr_code and is_grounded_in_ocr(curr_code, ocr_text)) else ""
