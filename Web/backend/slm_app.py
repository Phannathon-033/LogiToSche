...
def slm_extract(payload: SlmExtractRequest) -> dict[str, Any]:
    try:
        ...
        messages = [
            {"role": "system", "content": "You extract logistics document data. Return only valid JSON. Do not include markdown or explanations."},
...
def build_slm_prompt(payload: SlmExtractRequest) -> str:
    schema = {
        ...
        "fields": [{"sourceText": "source text from OCR", "field": "invoice_no", "value": "normalized value", "confidence": 0, "status": "success | review | error | processing"}],
        "confidence": {"overall": 0, "ocr": 0, "slm": 0, "mapping": 0, "completeness": 0},
        "review_items": [{"field": "truck_plate", "ocrValue": "raw OCR value", "slmValue": "normalized value", "confidence": 0, "status": "review"}],
    }
    return (
