...
export function OCRResultPanel({ text, spatialText, lines = [], onCopy }: OCRResultPanelProps) {
  const [viewMode, setViewMode] = useState<"spatial" | "raw">("spatial");
  const [copied, setCopied] = useState(false);
  const isLoading = text === "กำลังส่งไฟล์ไปยัง PaddleOCR Backend..." || text === "กำลังส่งไฟล์ไปยัง PaddleOCR GPU...";

  const hasSpatial = Boolean(spatialText || (lines && lines.length > 0 && lines.some((l) => l.position)));

  function handleCopy() {
    const textToCopy = viewMode === "spatial" && spatialText ? spatialText : text;
      ...
      {isLoading ? (
        <div className="flex h-[430px] min-h-[430px] flex-col items-center justify-center overflow-hidden rounded-xl border border-slate-700 bg-slate-900 shadow-inner">
          ...
          <h4 className="mb-2 font-mono text-sm font-bold tracking-wider text-emerald-400">
            EXTRACTING 2D SPATIAL OCR
          </h4>

          <div className="flex items-center gap-2">
            <span className="font-mono text-xs text-emerald-500/80">กำลังสกัดข้อความพร้อมพิกัดตำแหน่ง Bounding Boxes</span>
            ...
            {lines.map((line, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {line.confidence ? (
                    <span className="shrink-0 text-[10px] text-slate-500 opacity-60 group-hover:opacity-100">
